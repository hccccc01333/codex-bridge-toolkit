...
需要把多个本地交付物交给 ChatGPT 网页端时，使用三步显式流程：

```text
1. bridge_attachment_package：点名工作区内的文件，生成 .codex-bridge/attachments/*.zip
2. bridge_attachment_upload：将该 ZIP（或单个点名文件）放入当前已验证网页对话的编辑器
3. bridge_send：明确发送说明文字
```

第 2 步只把文件附到编辑器，不会隐式发送聊天消息。工具仅接受工作区内的常规文件、最多 100 个文件/500 MB，总是保留生成的 ZIP，不会覆盖已有同名 ZIP。

网页端文件反向交接同样需要显式选择：先调用 `bridge_attachment_list` 查看当前网页对话中可见的下载项，再用 `bridge_attachment_download` 指定 `attachment_id`。文件只会保存到当前工作区的 `.codex-bridge/downloads/`（或用户指定的工作区内目录），完成后返回相对路径、大小和 SHA-256。下载目录无法受控、附件列表变化或下载未完成时，桥接会暂停，不会点击第二次或换标签页。
...
| ChatGPT 网页附件交接 | ✅ | 显式 ZIP 打包、同一已验证对话上传、可见附件选择下载；失败即暂停 |
...
不要直接让桥接扫描目录。明确选择文件后：先打包，再上传到当前编辑器，最后再明确发送消息。网页端下载必须先列出附件再选择一个 `attachment_id`；如果 ChatGPT UI 没有可识别的下载控件或浏览器不能把下载保存到工作区，插件会暂停并保留原网页标签页。
