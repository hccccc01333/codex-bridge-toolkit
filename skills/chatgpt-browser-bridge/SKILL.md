...
The repository is an umbrella toolkit series. In addition to the bridge, it exposes a read-only Browser Watchdog, a read-only GitHub Workspace toolkit, an MCP host compatibility report, bounded local artifact discovery/context, explicit web-attachment handoff, and an experimental Web Session Swarm. Diagnostic/context tools never silently switch tabs, resend a possibly completed prompt, pull/push/commit, read unselected document bodies, upload files, or perform account actions. Attachment upload/download is separate: it requires explicit user-selected files or a visible attachment choice.
...
10. For multiple local files, call `bridge_attachment_package` only with explicitly selected relative workspace files; it creates a local ZIP and does not upload it. Call `bridge_attachment_upload` only after the user explicitly asks to add the selected file(s) to the exact bound web composer; it attaches but does not send a chat message. For web attachments, call `bridge_attachment_list` first and only call `bridge_attachment_download` after the user explicitly selects an `attachment_id`. Do not download to an unbounded location, scan arbitrary directories, upload unselected files, or retry an unconfirmed transfer.
...
- ZIP packaging is local-only and accepts explicitly selected regular files inside the workspace. Web upload requires an exact active destination and adds files to the composer only; a following message still requires `bridge_send`. Web download requires `bridge_attachment_list` plus one explicit `attachment_id`, and must use a controlled workspace-relative download directory. Any confirmation failure pauses the bridge without another click or a replacement tab.
