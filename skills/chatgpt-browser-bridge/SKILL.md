...
2. Call `chatgpt_browser_status` with the chosen `session_id`, then `chatgpt_browser_launch` if needed. The user signs in manually.
3. Call `chatgpt_browser_open` with the chosen `session_id` to connect or allocate its ChatGPT tab.
4. Use `chatgpt_browser_list_conversations` to inspect visible sidebar chats, then `chatgpt_browser_select_conversation` by exact title, ID, or URL when a specific brain session is needed. Use `chatgpt_browser_current_conversation` to confirm selection. Include the same `session_id` on every call.
5. Call `brain_plan` with the goal, relevant context, constraints, and an optional `max_rounds` from 1 to 50. The default is 20. Include the same `session_id` used for the tab.
6. Luna executes only the returned `task`, then calls `executor_report` with concise results. Do not send hidden reasoning, credentials, or unrelated workspace content.
7. Call `brain_review` to classify the report as `continue`, `completed`, `blocked`, or `repeated`, then `continue_task` to advance one round. It enforces the round limit and stops on completion, blocking, repetition, or the limit.
8. Use `brain_status` to inspect one session and `brain_reset` to reset it. Resetting does not delete browser tabs or session files.
